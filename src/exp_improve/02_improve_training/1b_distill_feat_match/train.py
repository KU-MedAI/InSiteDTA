...
def get_arguments():
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default=os.path.join(_SCRIPT_DIR, "../../ckpts/02_improve_training/1b_distill_feat_match"),
...
def train_model(
    ...
    for epoch in range(train_cfg["epochs"]):
        ...
        for i, sample in enumerate(tqdm(tr_loader)):
            ...
            # Teacher forward (clean input, same rotation) → extract bottleneck_p2l
            with torch.no_grad():
                with autocast(device_type=device.type):
                    _, _, teacher_feat = teacher(voxel, lig_data, return_features=True)
            ...
            with autocast(device_type=device.type):
                pred_poc, pred_aff, student_feat = model(voxel, lig_data, return_features=True)
                ...
                # Distillation loss: cosine similarity on bottleneck_p2l features
                # Flatten spatial dims: [B, C, D, H, W] → [B, C*D*H*W]
                t_flat = teacher_feat.detach().flatten(1)
                s_flat = student_feat.flatten(1)
                distill_loss = 1.0 - nn.functional.cosine_similarity(s_flat, t_flat, dim=1).mean()
...
def main():
    ...
    exp_name = f"distill-feat-match_{split_name}_s{train_cfg['seed']}_{timestamp}"
