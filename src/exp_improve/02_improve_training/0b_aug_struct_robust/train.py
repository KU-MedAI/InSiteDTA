...
def apply_voxel_jitter(voxel, pocket, max_shift, generator=None):
    """Randomly shift voxel and pocket in 3D space (zero-pad, no wrap-around).

    Simulates pocket center prediction error from p2rank / AlphaFold binding-site
    predictions.  voxel: (B, C, D, H, W), pocket: (B, 1, D, H, W).
    """
    _, _, D, H, W = voxel.shape
    p = max_shift
    sd = torch.randint(-p, p + 1, (1,), generator=generator).item()
    sh = torch.randint(-p, p + 1, (1,), generator=generator).item()
    sw = torch.randint(-p, p + 1, (1,), generator=generator).item()
    # F.pad order: (W_left, W_right, H_left, H_right, D_left, D_right)
    voxel_p = F.pad(voxel, (p, p, p, p, p, p), value=0.0)
    pocket_p = F.pad(pocket, (p, p, p, p, p, p), value=0.0)
    d0, h0, w0 = p + sd, p + sh, p + sw
    voxel = voxel_p[:, :, d0:d0 + D, h0:h0 + H, w0:w0 + W]
    pocket = pocket_p[:, :, d0:d0 + D, h0:h0 + H, w0:w0 + W]
    return voxel, pocket


def apply_gaussian_blur_3d(voxel, sigma):
    """Apply 3D Gaussian blur independently per channel.

    Simulates atomic coordinate uncertainty in predicted structures.
    voxel: (B, C, D, H, W).
    """
    radius = max(1, int(3 * sigma + 0.5))
    size = 2 * radius + 1
    x = torch.arange(-radius, radius + 1, dtype=torch.float32, device=voxel.device)
    g = torch.exp(-x ** 2 / (2 * sigma ** 2))
    g = g / g.sum()
    kernel = g[:, None, None] * g[None, :, None] * g[None, None, :]  # (size, size, size)
    C = voxel.shape[1]
    kernel = kernel.unsqueeze(0).unsqueeze(0).expand(C, 1, size, size, size).contiguous()
    return F.conv3d(voxel, kernel, padding=radius, groups=C)
...
def get_arguments():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Max voxel grid shift (in voxels) for pocket center jitter (0 to disable)",
    )
    parser.add_argument(
        "--jitter_prob",
        type=float,
        default=0.5,
        help="Probability of applying pocket center jitter per batch",
    )
    parser.add_argument(
        "--blur_sigma",
        type=float,
        default=0.8,
        help="Sigma for 3D Gaussian blur simulating coordinate uncertainty",
    )
    parser.add_argument(
        "--blur_prob",
        type=float,
        default=0.5,
        help="Probability of applying 3D Gaussian blur per batch",
    )
...
def train_model(
    ...
    for epoch in range(train_cfg["epochs"]):
        ...
        for i, sample in enumerate(tqdm(tr_loader)):
            ...
            # Pocket center jitter: random 3D spatial shift
            if train_cfg["jitter_max"] > 0 and torch.rand(1, generator=aug_generator).item() < train_cfg["jitter_prob"]:
                voxel, pocket = apply_voxel_jitter(voxel, pocket, train_cfg["jitter_max"], aug_generator)
            ...
            # Voxel-wise Gaussian noise
            if train_cfg["voxel_noise_std"] > 0:
                voxel = voxel + torch.randn_like(voxel) * train_cfg["voxel_noise_std"]

            # 3D Gaussian blur: simulates coordinate uncertainty in predicted structures
            if train_cfg["blur_prob"] > 0 and torch.rand(1, generator=aug_generator).item() < train_cfg["blur_prob"]:
                voxel = apply_gaussian_blur_3d(voxel, train_cfg["blur_sigma"])
...
def main():
    ...
    exp_name = f"aug-struct-robust_{split_name}_s{train_cfg['seed']}_{timestamp}"
